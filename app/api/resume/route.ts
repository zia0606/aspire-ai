...
function countMetrics(text: string) {
  const matches = text.match(/(?:\b\d+(?:\.\d+)?%|₹\s?\d+[\d,.]*|\$\s?\d+[\d,.]*|\b\d+[,.]?\d*\+?\s?(?:users|customers|clients|projects|hours|days|weeks|months|sales|leads|downloads|requests|records|students|members)\b)/gi);
...
function localAnalysis(text: string, profile: Profile, career: CareerDefinition) {
  ...
  const keywordCoverage = career.skills.length
    ? Math.round((detectedSkills.length / career.skills.length) * 100)
    : 0;
  ...
  const resumeScore = Math.min(
    100,
    Math.round(keywordCoverage * 0.4 + sectionCoverage * 0.25 + evidenceScore * 0.2 + lengthScore * 0.15),
  );
  ...
  if (missingSkills.length) {
    suggestions.push(`If these skills are genuinely part of your experience, show evidence for ${missingSkills.slice(0, 3).join(", ")}. Do not add skills you cannot explain in an interview.`);
  }
  if (missingSections.length) {
    suggestions.push(`Strengthen the resume structure by adding or clearly labeling: ${missingSections.slice(0, 3).join(", ")}.`);
  }
  if (metricEvidence < 2) {
    suggestions.push("Add measurable outcomes where truthful: percentages, time saved, users served, projects completed, revenue, accuracy or other concrete results.");
  }
  if (actionEvidence < 5) {
    suggestions.push("Use stronger achievement verbs at the start of bullets, such as built, developed, analyzed, improved, automated or led.");
  }
  if (wordCount < 220) {
    suggestions.push("The resume looks very short. Add stronger project, skill and achievement evidence rather than filler.");
  } else if (wordCount > 1050) {
    suggestions.push("The resume is long. Remove repetition and keep the most relevant evidence for the target career.");
  }
  if (!suggestions.length) {
    suggestions.push("Your structure and tracked career keywords are strong. Focus next on sharper outcomes, cleaner bullet writing and tailoring each application to the actual job description.");
  }
...
async function getCoachNote(
  ...
  analysis: ReturnType<typeof localAnalysis>,
) {
...
export async function POST(request: Request) {
  ...
  if (!isProfile(body.profile)) {
    return Response.json({ error: "Complete the Aspire AI assessment first." }, { status: 400 });
  }

  const resumeText = typeof body.resumeText === "string" ? body.resumeText.trim() : "";
  if (resumeText.length < 120) {
    return Response.json({ error: "Paste more of your resume before analyzing it." }, { status: 400 });
  }
  if (resumeText.length > 20000) {
    return Response.json({ error: "Resume text is too long. Keep it under 20,000 characters." }, { status: 400 });
  }

  const career = careerCatalog[body.profile.career];
  if (!career) {
    return Response.json({ error: "Career profile is not recognized. Retake the assessment once." }, { status: 400 });
  }

  const analysis = localAnalysis(resumeText, body.profile, career);
